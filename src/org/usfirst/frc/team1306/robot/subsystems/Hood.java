...
public class Hood extends Subsystem {
	...
	public Hood() {
		...
		SmartDashboard.putBoolean("use hood setpoint", SmartDashboard.getBoolean("use hood setpoint", false));
	...
	private double getAngleForDistance(double distance) {
		...
		if (SmartDashboard.getBoolean("use hood setpoint",true)) {
			return SmartDashboard.getNumber("hood set height",0);
		}
	...
	public void setTarget(HoodTarget target) {
		...
		if (target == null) {
			...
			if (target.equals(HoodTarget.AUTO)) {
					...
					height = SmartDashboard.getNumber("hood set height",0);
